(module kor-blocks-policy GOVERNANCE
    ...
    (use marmalade.ledger) ;; I don't think you need to specify this $
    (use coin)    ;; I don't think you need to specify this $
  ...
  ;;Constants
    ...
    (defconst KOR_BANK  "k:a6f4b28bcdbdbf6d557dc2d3a0c1d80893e864d7ae38b5f4f007d7197ca4500a")  ; Make this changeable, don't hard code it $
    ;(defconst KOR_BANK  "k:c7f39d3fa10cd08ae1bcc22fb8ddae65798b9dfc261c392873001035bae19a10") FOR TESTNET
    (defconst DEV_BANK  "k:b2e645cc9f86f332f3d2d345825b339e83a0d38e9ec20096ad14099e24789c53") ; Make this changeable, don't hard code it $
    (defconst OPERATOR_ACC  "k:96a46fe541b6e1370f7a4b8a20599002f87250bd7940725cf1804843a1d8bdd8") ; Make this changeable, don't hard code it $
  ...
  ;;Schemas and tables
    ...
    (defschema collection
      ...
      kor-account:string 
      kor-guard:guard ; Verify you actually need this $
      dev-account:string
      dev-guard:guard  ; Verify you actually need this $
      dev-fee:decimal
      tokens:[string] 
      sold-tokens:[string]  ; You don't need to store this, just use math operators $
      unsold-tokens:[string] ; You don't need to store this, just use math operators $
      index:integer
      tiers:[object:{tier}]
    ...
    (defschema creators
      ...
      address:string  ; may want to consider if you support multiple royalty recipients $
      blockchain:string)
    ...
    (defcap INIT_COLLECTION:bool (collection-id:string collection-size:integer fungible:module{fungible-v2} price:decimal operator:string)
      ...
      @event
      (fungible::details operator))  ; I think this needs a further deep dive, I would need to repl it and test $
    ...
    (defcap CREATE_TOKEN (mint-guard:guard account:string)
      @doc "Capability that enforces a valid account and guard to create the token"
        ...
        (enforce-keyset mint-guard)) ; mental note for me to go back and check this in repl $
  ...
  ;; -----------------------------------------------------------------------------
    ...
    (defun init-collection:bool
      ( collection-id:string
        ...
        (with-capability (GOVERNANCE) ; Change this off the governance cap and split with some type of OPS $
        ...
        ; I have some code that's set up to allow your collection creator with the ability to modify whitelist, if you end up
        ; allowing others to interact with this outside of OPERATOR. $
        ...
        (with-capability (END_SALE)
          (with-capability (INTERNAL)
            (with-read collections collection-id {
              ...
              (coin.transfer account creator (* amount (floor (* (get-kda-price price) (- 1.0 dev-fee))4))) ; Choose decimal or integer, not both for math operations $
              (coin.transfer account dev (* amount (floor(* (get-kda-price price) dev-fee)4))) ; Choose decimal or integer, not both for math operations $
