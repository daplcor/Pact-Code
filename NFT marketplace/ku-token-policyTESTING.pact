...
(defun enforce-init:bool
  ...
  (let* ( (spec:object{policy-schema} (read-msg TOKEN_SPEC))
         ...
         ; (creator-guard:guard (create-guard-for-creator creator))
...
;; Guard Creation
(defun create-guard-for-creator (creator:string)
  (enforce (not (null creator)) "Creator address must not be empty")
  (create-user-guard creator)
)
