(module ku-inits GOVERNANCE
    @doc "This code is meant to be copy-pasted into a different smart contract."
  
    (define-keyset "free.ku-inits-admin" (read-keyset "ku-inits-admin"))
    (define-keyset "free.ku-inits-ops" (read-keyset "ku-inits-ops"))

    ;; -------------------------------
    ;; Governance and Permissions
  
    (defcap GOVERNANCE ()
    (enforce-guard (keyset-ref-guard "free.ku-inits-admin"))
    (compose-capability (OPS_INTERNAL))
    ) 

    (defcap OPS ()
    (enforce-guard (keyset-ref-guard "free.ku-inits-ops"))
    (compose-capability (OPS_INTERNAL))
    )
